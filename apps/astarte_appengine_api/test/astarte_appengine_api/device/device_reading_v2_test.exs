...
defmodule Astarte.AppEngine.API.Device.DeviceReadingV2Test do
  ...
  describe "get_interface_value" do
    setup context do
      on_exit(fn -> clean_device_saved_data(context.astarte_instance_id, context.realm_name) end)
    end
    ...
    property "returns the value from interface path ",
             context do
      %{
        realm_name: realm_name,
        astarte_instance_id: astarte_instance_id,
  ...
  defp clean_device_saved_data(astarte_instance_id, realm_name) do
    Repo.query!("TRUNCATE #{astarte_instance_id <> realm_name}.individual_properties")
    Repo.query!("TRUNCATE #{astarte_instance_id <> realm_name}.individual_datastreams")
  end
